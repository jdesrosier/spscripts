Param(
		...
		#Example: ExcellianNetSiteBuilder -webTitle "Excellian.net" -webName "" -siteCollectionUrl "http://www-prod.excellian.net" -siteGUID "32ddcc11-7b9c-4053-a360-4502ccdebd12"
	...
	Function Create-DocLibrary([string]$libraryName, [string]$libraryDesc, [string]$topicGUID)
	{
		...
		if($spContainer -ne $null) {
         ...
         Write-Host ("spWeb: {0}" -f $spWeb)
         Write-Host ("spContainer: {0}" -f $spContainer)
		...
		$AKNDocCT = $spSite.RootWeb.ContentTypes["ENet Standard Document"]
      		
		# Check to see if content type exists already
		if ($spContainer.ContentTypes["ENet Standard Document"] -eq $null)
       	 	{
		...
		Set-DefaultColumnValue "ENet Site" $siteValStr
		...
		Add-ViewColumns "Epic Application"
		Add-ViewColumns "Modified By"
		Add-ViewColumns "Modified"
		Add-ViewColumns "Browsable"
		

		# Set the view: group by and order by settings
		#$view.Query = "<GroupBy Collapse=""TRUE"" GroupLimit=""100""> <FieldRef Name=""Subtopic"" Ascending=""FALSE""/> </GroupBy> <OrderBy> <FieldRef Name=""Modified"" Ascending=""FALSE"" /></OrderBy>"
    	$viewRowLimit = 50
        $view.Update()
	...
	Function Create-LinkList([string]$listName, [string]$listDesc, [string]$topicGUID)
	{
		...
		if($spContainer) {
			...
			$spContainer = $spWeb.GetList($spWeb+"/lists/"+"$linkListName")
#$spContainer = $spWeb.GetList($spWeb.ServerRelativeUrl+"/lists/"+"$linkListName")
		...
		$AKNDocCT = $spSite.RootWeb.ContentTypes["ENet Site Links"]
...
Function Create-Subsite([string]$webUrl, [string]$webTitle)
{
    ...
    else
    {
	    ...
	    $spFile.Properties["PublishingPageLayout"] = $siteCollectionUrl + "/_catalogs/masterpage/ENetPrimary.aspx, ENet Primary"
	    $spFile.Update()
...
$webUrl = $siteCollectionUrl + $webPageUrl
...
Write-Host "SiteColl: " $siteCollectionUrl
Write-Host "GUID: " $siteGUID